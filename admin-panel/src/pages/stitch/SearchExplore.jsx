    ...
    <div className="py-8 px-md md:px-lg max-w-container-max mx-auto min-h-screen">
      ...
      <section className="mb-lg">
        <div className="relative group">
          ...
          <input 
            className="w-full h-12 pl-12 pr-12 bg-surface-container-low border-none rounded-xl focus:ring-2 focus:ring-trust-blue font-body-lg text-body-lg transition-all shadow-sm" 
            placeholder="Search for bread, coffee, or local shops..." 
            type="text" 
            value={searchTerm} 
          ...
          <div className="absolute inset-y-0 right-0 pr-md flex items-center">
            {searchTerm ? (
              <button onClick={() => setSearchTerm('')} className="p-base hover:bg-surface-container-high rounded-full transition-colors active:scale-95">
      ...
      {/* Search Results List (Only shown when user types a search query) */}
      {searchTerm.trim() !== '' && (
        <section className="mb-lg">
          <h2 className="font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-md">
            Search Results ({products.length})
          </h2>
          {products.length === 0 ? (
            <p className="text-on-surface-variant font-body-md py-4">No products found matching "{searchTerm}".</p>
          ) : (
