  ...
  {visibleNotifications.length === 0 ? (
    ...
    visibleNotifications.map((n) => {
      ...
      if (n.type === "SUPPORT_REPLY") {
        return (
          <div key={n.id} className={`group bg-surface-container-low p-md rounded-xl shadow-sm border-l-4 border-marketplace-orange hover:bg-surface-container-high transition-all ${n.isRead ? 'opacity-70' : ''}`}>
            <div className="flex gap-md">
              <div className="w-12 h-12 rounded-full bg-marketplace-orange/10 flex items-center justify-center flex-shrink-0 text-marketplace-orange">
                <span className="material-symbols-outlined">support_agent</span>
              </div>
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <h3 className="font-title-md text-body-lg font-bold text-on-surface">{n.title}</h3>
                  {!n.isRead && <span className="font-label-sm text-[10px] text-marketplace-orange font-bold">New</span>}
                </div>
                <p className="font-body-md text-body-md text-on-surface-variant mt-1">{n.message}</p>
                <div className="mt-md flex gap-sm">
                  <button 
